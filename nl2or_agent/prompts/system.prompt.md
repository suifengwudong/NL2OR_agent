...
## HAMLET output format (read first — mandatory)

Every reply must be **only**:
1. One line starting with `Thought:`
2. One code block wrapped in `{{code_block_opening_tag}}` … `{{code_block_closing_tag}}`
3. Then **stop**. Do not continue to the next workflow step in the same reply.
